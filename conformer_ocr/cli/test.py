...
def test(ctx, batch_size, model, evaluation_files, device, pad, workers,
    ...
    from kraken.lib.dataset import (GroundTruthDataset, ImageInputTransforms,
                                    PolygonGTDataset,
    ...
    from conformer_ocr.dataset import ArrowIPCRecognitionDataset, collate_sequences
    ...
    if nn.context_token_input:
        message('Extracting semantic context token fields')
        dataset_kwargs['semantic_token_fields'] = nn.semantic_token_fields
        message(f'Found {nn.semantic_token_fields}')
        ...
        with KrakenProgressBar() as progress:
            ...
            for batch in ds_loader:
                ...
                if nn.context_token_input:
                    ...
                    semantic_token = semantic_token.unsqueeze(-1).expand(-1, -1, im.shape[-1])
                    im = torch.cat([im.squeeze(1), semantic_token], dim=1).unsqueeze(1)
                text = batch['target']
                lens = batch['seq_lens']
                im = im.to(device)
                try:
